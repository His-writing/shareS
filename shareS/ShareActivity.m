...
//分享按钮_间隔_宽度
//#define SHAREBUTTON_INTERVAL_WIDTH              11

#define SHAREBUTTON_INTERVAL_WIDTH               (kDeviceWidth-50*4)/5
//分享按钮_间隔_高度
...
//沙间_间隔_宽度
#define SHARETITLE_INTERVAL_WIDTH               (kDeviceWidth-50*4)/5
//分享标题_间隔_高度 =分享按钮_间隔_高度+分享按钮_宽度
...
//#define TITLE_INTERVAL_HEIGHT                   (kDeviceWidth-50*4)/5
...
#define TITLE_INTERVAL_WIDTH                    (kDeviceWidth-TITLE_WIDTH)/2
...
@property (nonatomic,assign) id<ShareActivityDelegate>delegate;
    ...
    if (shareButtonImagesNameArray) {
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                //计算出行数，与列数
                int column = (int)ceil((float)(i)/5); //行
                int line = (i)%5; //列
                if (line == 0) {
                    line = 5;
                }
                
                NSLog(@"column--%d,line--%d",column,line);
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                //计算出行数，与列数
                int column = (int)ceil((float)(j)/5); //行
                int line = (j)%5; //列
                if (line == 0) {
                    line = 5;
                }
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake((kDeviceWidth-BUTTON_WIDTH)/2, BUTTON_INTERVAL_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)];
